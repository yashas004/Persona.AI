...
def process_face_landmarks(image, face_mesh):
    ...
    if results.multi_face_landmarks:
        for face_landmarks in results.multi_face_landmarks:
            ...
            facial_metrics["eye_movement"] = max(1.0, eye_distance * 100)  # Ensure minimum value of 1.0
            facial_metrics["mouth_movement"] = max(1.0, mouth_openness * 100)  # Ensure minimum value of 1.0
            facial_metrics["head_position"] = max(1.0, (face_landmarks.landmark[0].z + 0.5) * 100)  # Ensure minimum value of 1.0
            facial_metrics["expression_diversity"] = max(2.0, facial_metrics["expression_diversity"])  # Already has minimum value of 2.0
            
    else:
        # If no face detected, ensure minimum values
...
def analyze_audio(audio_file_path: str) -> Dict:
    ...
    pitch_variation = np.std(pitches[pitches > 0]) if np.any(pitches > 0) else 0.1  # Default if no pitch detected
    
    # Speech rate (simplified)
    zero_crossings = librosa.zero_crossings(y)
    speech_rate = sum(zero_crossings)
    
    # Create audio metrics dictionary with minimum values
    audio_metrics = {
        "volume": max(1.0, min(100, energy * 1000)),  # Between 1.0 and 100
        "pitch_variation": max(1.0, min(100, pitch_variation * 10)),  # Between 1.0 and 100
        "speech_rate": max(1.0, min(100, speech_rate / 1000)),  # Between 1.0 and 100
        "clarity": max(1.0, random.uniform(60, 95))  # Between 1.0 and 95
    }
...
def main():
    ...
    with tab2:
        if st.session_state.history:
            ...
            # Prepare data for charts - ensure no zero values
            facial_data = {k: max(1.0, v) for k, v in latest_session['facial_metrics'].items()}
            audio_data = {k: max(1.0, v) for k, v in latest_session['audio_metrics'].items()}
        ...
        if st.session_state.history:
            ...
            for session in st.session_state.history:
                progress_data.append({
                    ...
                    'Voice Score': max(1.0, session['feedback']['voice_score'])
                })
